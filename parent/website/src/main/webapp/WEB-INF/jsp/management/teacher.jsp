...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		window.grid = new Grid({
			id:"teachers",
			url:"${context}/management/teacher.do",
	...
	function removeCourse(){
		...
		$.ajax({url:"${context}/management/delete.do",data:{id:selectedRows[0].id},success:function(result){
			if(result.success){
				alert("删除成功");
				window.location.href="${context}/management/teacher.do";
			}else{
...
<h1>教师信息管理</h1>
<div>
	<form class="form-inline" action="${context}/management/teacher.do" method="POST">
		<div class="form-group">
			<label for="exampleInputName2">教师姓名: </label> <input type="text"
				class="form-control" id="name" name="name" value="${name }" placeholder="请输入教师姓名">
		</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<form id="updateCourse">
					...
					<div class="row">
						<div class="form-group col-md-8">
							<label for="gander">性别: </label> <select id="ucGander" class="form-control" uv-onChange="true" nv="true" uv-message="请选择性别" uv-method="validateSelect" name="gander">
							<option value="-1">--请选择--</option>
							<option value="男">男</option>
							<option value="女">女</option>
						</select>
